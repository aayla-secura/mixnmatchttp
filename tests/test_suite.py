...
class TestBaseMeta(unittest.TestCase):
    def test_endpoint_conversion(self):
        self.assertIsInstance(
                ...
                endpoints.Endpoint,
                msg="BaseMeta failed to convert _endpoints to the appropriate class")

    def test_template_conversion(self):
        self.assertIsInstance(
                TestHTTPRequestHandler._template_pages,
                DictNoClobber,
                msg="BaseMeta failed to convert _template_pages to the appropriate class")

    def test_endpoint_inheritance(self):
        self.assertIn('logout',
                TestHTTPRequestHandler._endpoints.keys(),
                msg="BaseMeta failed to include parent's endpoints")

    def test_endpoint_inheritance_existing(self):
        self.assertIn('test_sub',
                TestHTTPRequestHandler._endpoints['login'].keys(),
                msg="BaseMeta overwrote current class endpoint with parent's endpoint")

    def test_template_inheritance(self):
        self.assertIn('default',
                TestHTTPRequestHandler._template_pages.keys(),
                msg="BaseMeta failed to include parent's template pages")
